...
@app.get('/')
def read_root():
    return {'app': 'DataFlow Python Engine', 'status': 'ok', 'engine': 'Pandas + FastAPI + Groq AI'}
...
def run_pipeline_python(pipeline: PipelineRequest):
    nodes_dict = [n.model_dump() for n in pipeline.nodes]
    edges_dict = [e.model_dump() for e in pipeline.edges]
    try:
        res = execute_pipeline(nodes_dict, edges_dict)
        return res
    except Exception as e:
        raise HTTPException(status_code=400, detail=str(e))


@app.post('/ai/generate-pipeline')
def generate_ai_pipeline(req: AIPromptRequest):
    if not req.prompt or not req.prompt.strip():
        raise HTTPException(status_code=400, detail="Prompt is required.")
    try:
        res = generate_pipeline_ai(req.prompt.strip(), req.apiKey)
        return res
    except Exception as e:
