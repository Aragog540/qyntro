...
const NODE_DOCS = [
  ...
  },
  {
    ...
    badgeColor: 'var(--color-io)',
    icon: '🗄️',
    summary: 'Query live SQL databases (PostgreSQL, MySQL, SQLite) or public SQL sandbox via SQL queries.',
    inputs: 'None (Source Node)',
    outputs: '1 Query Result DataFrame',
    fields: [
      { name: 'Database Engine', desc: 'PostgreSQL Server, MySQL Server, SQLite / DuckDB, or Public SQL Sandbox.' },
      { name: 'Connection URL', desc: 'Database connection URI (e.g., postgresql://user:pass@localhost:5432/db).' },
      { name: 'SQL Query', desc: 'Executable SQL SELECT query string.' },
    ],
    example: 'Set Database Engine to "PostgreSQL" and query to "SELECT * FROM orders WHERE status = \'paid\'".',
  },
  {
    type: 'apiFetch',
    ...
    badgeColor: 'var(--color-io)',
    icon: '🌐',
    summary: 'Fetch live JSON or CSV data directly from external REST API endpoints in real-time.',
    inputs: 'None (Source Node)',
    outputs: '1 API Response DataFrame',
    fields: [
      { name: 'Endpoint URL', desc: 'Remote HTTP REST API URL.' },
      { name: 'HTTP Method', desc: 'GET or POST request method.' },
    ],
    example: 'Set Endpoint URL to "https://jsonplaceholder.typicode.com/users" to fetch live JSON records.',
  },
  {
    type: 'preview',
    label: 'Preview Data',
