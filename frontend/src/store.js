...
export const useStore = create((set, get) => ({
  ...
  setGroqApiKey: (key) => {
    localStorage.setItem('qyntro-groq-key', key);
    set({ groqApiKey: key });
  },
  ...
  loadAIPipeline: (aiResult) => {
    if (!aiResult?.nodes?.length) return;
    const typeCounts = {};
    const nodes = aiResult.nodes.map(n => {
    ...
    const edges = (aiResult.edges || []).map((e, idx) => ({
      id: e.id || `e-ai-${idx}`,
      source: e.source,
      target: e.target,
      sourceHandle: `${e.source}-output`,
      targetHandle: `${e.target}-input`,
      type: 'flow',
      style: { stroke: 'var(--color-border-hover)', strokeWidth: 2 },
    }));
